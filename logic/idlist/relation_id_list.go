...
func GetRelationIDList(ctx context.Context, entityID, scene string, routeInfo *model.RelationIDRouteDTO,
	...
	if routeInfo.Mode == constant.Offset {
		...
		return nil, errs.Newf(errcode.WujiRelationIDConfigError, "relation conf error, mod error, routeinfo:%+v",
			routeInfo)
	}
	// TODO 异步存入缓存
	return itemDTOs, err
...
// getRelationIDByOffset 根据offset的形式请求请求关系链信息
...
// getRelationIDByPageContext 根据offset的形式请求请求关系链信息
